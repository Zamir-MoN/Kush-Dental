...
import { ArrowUpRight, Calendar, Sparkles, RefreshCw, CheckCircle2 } from 'lucide-react';
...
export const BlogList = ({ searchQuery, setSearchQuery, blogPosts }: BlogListProps) => {
  ...
  const displayGridPosts = useMemo(() => {
    ...
    if (isSinglePostCategory && featuredPost) {
      return blogPosts.filter((p: any) => p.id !== featuredPost.id).slice(0, 3);
    }
    return [];
  }, [filteredPosts, isSinglePostCategory, featuredPost, blogPosts]);
